...
class ModelReportCustomer extends Model {
	...
	public function getOrders($data = array()) {
		$sql = "SELECT c.customer_id, CONCAT(c.lastname, ' ', c.firstname, ' ', c.middlename) AS customer, c.email, cgd.name AS customer_group, c.status, o.order_id, SUM(op.quantity) as products, SUM(o.total) AS total FROM `" . DB_PREFIX . "order` o LEFT JOIN `" . DB_PREFIX . "order_product` op ON (o.order_id = op.order_id) LEFT JOIN `" . DB_PREFIX . "customer` c ON (o.customer_id = c.customer_id) LEFT JOIN `" . DB_PREFIX . "customer_group_description` cgd ON (c.customer_group_id = cgd.customer_group_id) WHERE o.customer_id > 0 AND cgd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
		...
		if (!empty($data['filter_customer'])) {
			$sql .= " AND CONCAT(c.lastname, ' ', c.firstname, ' ', c.middlename) LIKE '" . $this->db->escape($data['filter_customer']) . "'";
		}
	...
	public function getRewardPoints($data = array()) {
		$sql = "SELECT cr.customer_id, CONCAT(c.lastname, ' ', c.firstname, ' ', c.middlename) AS customer, c.email, cgd.name AS customer_group, c.status, SUM(cr.points) AS points, COUNT(o.order_id) AS orders, SUM(o.total) AS total FROM " . DB_PREFIX . "customer_reward cr LEFT JOIN `" . DB_PREFIX . "customer` c ON (cr.customer_id = c.customer_id) LEFT JOIN " . DB_PREFIX . "customer_group_description cgd ON (c.customer_group_id = cgd.customer_group_id) LEFT JOIN `" . DB_PREFIX . "order` o ON (cr.order_id = o.order_id) WHERE cgd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
	...
	public function getTotalRewardPoints($data = array()) {
		...
		if (!empty($data['filter_customer'])) {
			$implode[] = "CONCAT(c.lastname, ' ', c.firstname, ' ', c.middlename) LIKE '" . $this->db->escape($data['filter_customer']) . "'";
		}
	...
	public function getCredit($data = array()) {
		$sql = "SELECT ct.customer_id, CONCAT(c.lastname, ' ', c.firstname, ' ', c.middlename) AS customer, c.email, cgd.name AS customer_group, c.status, SUM(ct.amount) AS total FROM `" . DB_PREFIX . "customer_transaction` ct LEFT JOIN `" . DB_PREFIX . "customer` c ON (ct.customer_id = c.customer_id) LEFT JOIN `" . DB_PREFIX . "customer_group_description` cgd ON (c.customer_group_id = cgd.customer_group_id) WHERE cgd.language_id = '" . (int)$this->config->get('config_language_id') . "'";
	...
	public function getCustomerSearches($data = array()) {
		$sql = "SELECT cs.customer_id, cs.keyword, cs.category_id, cs.products, cs.ip, cs.date_added, CONCAT(c.lastname, ' ', c.firstname, ' ', c.middlename) AS customer FROM " . DB_PREFIX . "customer_search cs LEFT JOIN " . DB_PREFIX . "customer c ON (cs.customer_id = c.customer_id)";
