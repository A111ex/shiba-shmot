...
class ModelReportAffiliate extends Model {
	public function getCommission($data = array()) {
		$sql = "SELECT at.affiliate_id, CONCAT(a.lastname, ' ', a.firstname, ' ', a.middlename) AS affiliate, a.email, a.status, SUM(at.amount) AS commission, COUNT(o.order_id) AS orders, SUM(o.total) AS total FROM " . DB_PREFIX . "affiliate_transaction at LEFT JOIN `" . DB_PREFIX . "affiliate` a ON (at.affiliate_id = a.affiliate_id) LEFT JOIN `" . DB_PREFIX . "order` o ON (at.order_id = o.order_id)";
	...
	public function getProducts($data = array()) {
		$sql = "SELECT at.product_id, CONCAT(a.lastname, ' ', a.firstname, ' ', a.middlename) AS affiliate, a.email, a.status, SUM(at.amount) AS commission, COUNT(o.order_id) AS orders, SUM(o.total) AS total FROM " . DB_PREFIX . "affiliate_transaction at LEFT JOIN `" . DB_PREFIX . "affiliate` a ON (at.affiliate_id = a.affiliate_id) LEFT JOIN `" . DB_PREFIX . "order` o ON (at.order_id = o.order_id) LEFT JOIN " . DB_PREFIX . "product";
	...
	public function getAffiliateActivities($data = array()) {
		...
		if (!empty($data['filter_affiliate'])) {
			$implode[] = "CONCAT(a.lastname, ' ', a.firstname, ' ', a.middlename) LIKE '" . $this->db->escape($data['filter_affiliate']) . "'";
		}
