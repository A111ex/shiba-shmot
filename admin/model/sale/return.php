...
class ModelSaleReturn extends Model {
	...
	public function getReturn($return_id) {
		$query = $this->db->query("SELECT DISTINCT *, (SELECT CONCAT(c.lastname, ' ', c.firstname, ' ', c.middlename) FROM " . DB_PREFIX . "customer c WHERE c.customer_id = r.customer_id) AS customer FROM `" . DB_PREFIX . "return` r WHERE r.return_id = '" . (int)$return_id . "'");

		return $query->row;
	}

	public function getReturns($data = array()) {
		$sql = "SELECT *, CONCAT(r.lastname, ' ', r.firstname, ' ', r.middlename) AS customer, (SELECT rs.name FROM " . DB_PREFIX . "return_status rs WHERE rs.return_status_id = r.return_status_id AND rs.language_id = '" . (int)$this->config->get('config_language_id') . "') AS status FROM `" . DB_PREFIX . "return` r";
		...
		if (!empty($data['filter_customer'])) {
			$implode[] = "CONCAT(r.lastname, ' ', r.firstname, ' ', r.middlename) LIKE '" . $this->db->escape($data['filter_customer']) . "%'";
		}
