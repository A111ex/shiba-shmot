...
class ModelSaleRecurring extends Model {
	public function getRecurrings($data) {
		$sql = "SELECT `or`.order_recurring_id, `or`.order_id, `or`.reference, `or`.`status`, `or`.`date_added`, CONCAT(`o`.lastname, ' ', `o`.firstname, ' ', `o`.middlename) AS customer FROM `" . DB_PREFIX . "order_recurring` `or` LEFT JOIN `" . DB_PREFIX . "order` `o` ON (`or`.order_id = `o`.order_id)";
		...
		if (!empty($data['filter_customer'])) {
			$implode[] = "CONCAT(o.lastname, ' ', o.firstname, ' ', o.middlename) LIKE '" . $this->db->escape($data['filter_customer']) . "%'";
		}
