	...
	public function send_new_products() {
		...
		if (!isset($_GET['pass']) || $_GET['pass'] != 'send_new_products') {
			...
			if ($emails) {
				...
				$message .= '  <head>' . "\n";
				$message .= '    <title>Новинки на сайте ' . $store_name . '</title>' . "\n";
				...
				$message .= '      <a href="' . $siteUrl . 'krossovki/" style="color: #333; text-decoration: none; font-size: 16px; padding: 10px;" >Кроссовки</a>' . "\n";
				$message .= '      <a href="' . $siteUrl . 'odezhda/" style="color: #333; text-decoration: none; font-size: 16px; padding: 10px;">Одежда</a>' . "\n";
				$message .= '      <a href="' . $siteUrl . 'aksessuary/" style="color: #333; text-decoration: none; font-size: 16px; padding: 10px;">Аксессуары</a>' . "\n";

				$message .= '      <a style="text-decoration: none;" href="' . $siteUrl . '"><h1 style="color: #333; margin-top: 40px; margin-bottom: 20px; text-transform: uppercase; font-size: 24px;">Новинки на сайте ' . $store_name . '</h1></a>' . "\n";
				...
				foreach ($emails as $email) {
					if (filter_var($email, FILTER_VALIDATE_EMAIL)) {
						...
						$mail->setSubject(html_entity_decode('Новинки на сайте ' . ucfirst($store_name), ENT_QUOTES, 'UTF-8'));
