...
class ControllerExtensionModuleProductTab extends Controller {
	...
	public function index() {   
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			...
			$this->cache->delete('product_featured');
			$this->cache->delete('product_latest');
		...
		$data['entry_product_featured'] = $this->language->get('entry_product_featured');
		$data['entry_product_latest'] = $this->language->get('entry_product_latest');
		...
		if (!isset($this->request->get['module_id'])) {
			...
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/module/product_tab', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL')
			);			
		}
		
		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/product_tab', 'token=' . $this->session->data['token'], 'SSL');
		} else {
			$data['action'] = $this->url->link('extension/module/product_tab', 'token=' . $this->session->data['token'] . '&module_id=' . $this->request->get['module_id'], 'SSL');
		}
		...
		if (isset($this->request->post['product_featured'])) {
			$products_featured = $this->request->post['product_featured'];
		} elseif (!empty($module_info)) {
			$products_featured = $module_info['product_featured'];
		} else {
		...
		if (isset($this->request->post['product_latest'])) {
			$products_latest = $this->request->post['product_latest'];
		} elseif (!empty($module_info)) {
			$products_latest = $module_info['product_latest'];
		} else {
			$product_latest = array();
		}	
		
		if ($products_latest) {
			foreach ($products_latest as $product_id) {
				$product_info = $this->model_catalog_product->getProduct($product_id);
		...
		if (isset($this->request->post['product_latest_manual'])) {
			$data['product_latest_manual'] = $this->request->post['product_latest_manual'];
		} elseif (!empty($module_info)) {
			$data['product_latest_manual'] = $module_info['product_latest_manual'];
		} else {
			$data['product_latest_manual'] = 0;
		}					
		...
		$this->template = 'extension/module/product_tab.tpl';
