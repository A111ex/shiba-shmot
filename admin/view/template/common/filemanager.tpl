      ...
      <div class="row">
        ...
        <div class="col-sm-3 col-xs-6 text-center">
          ...
          <a href="<?php echo $image['href']; ?>" class="thumbnail">
		  	<?php if (mb_stripos($image['href'], '.mp4') !== false) { ?>
				<video autoplay="" loop="" muted="" playsinline="" width="100" height="100"  src="<?php echo $image['href']; ?>"></video>
			<?php } else { ?>
...
$('a.thumbnail').on('click', function(e) {
	...
	<?php if ($thumb) { ?>
		<?php if ($thumb == 'thumb-video') { ?>			
			if ($(this).find('video').attr('src').toUpperCase().includes('.MP4') ) {
				$('#<?php echo $thumb; ?>').find('video').attr('src', $(this).find('video').attr('src'));
			}			
			<?php } else {?>
				if (!$(this).find('img').attr('src').toUpperCase().includes('.MP4') ) {
