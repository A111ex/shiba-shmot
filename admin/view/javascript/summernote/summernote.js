...
(function (factory) {
  ...
  var Editor = function (context) {
    ...
    this.insertImage = function (src, param) {
      ...
      var extension = src.match(mp4RegExp);
      if (extension) {
        var $node =  $('.summernote').summernote('module', 'videoDialog').createBannerVideoNode(src);
  ...
  var VideoDialog = function (context) {
    ...
    this.createBannerVideoNode = function (url) { // BMV
      // video url patterns(youtube, instagram, vimeo, dailymotion, youku, mp4, ogg, webm)
      var ytRegExp = /^(?:https?:\/\/)?(?:www\.)?(?:youtu\.be\/|youtube\.com\/(?:embed\/|v\/|watch\?v=|watch\?.+&v=))((\w|-){11})(?:\S+)?$/;
      var ytMatch = url.match(ytRegExp);
      ...
      if (ytMatch && ytMatch[1].length === 11) {
        ...
        $video = $('<iframe>')
            .attr('frameborder', 0)
            .attr('src', '//www.youtube.com/embed/' + youtubeId)
            .attr('autoplay', '').attr('loop', '').attr('muted', '').attr('playsinline', '');
      } else if (igMatch && igMatch[0].length) {
        $video = $('<iframe>')
            .attr('frameborder', 0)
            .attr('src', igMatch[0] + '/embed/')
      ...
      $video.addClass('video-banners');
      return $video[0];

      // var $div1 = $('<div>').addClass('row').addClass('video-banners');
      // var $div2 = $('<div>').addClass('col-sx-12');
      // $div1[0].appendChild($div2[0]).appendChild($video[0]); 
