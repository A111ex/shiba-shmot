...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		if ($query->num_rows) {
			return array(
				...
				'sizes_help_image'    => $query->row['sizes_help_image'],
	...
	public function getLatestProducts($limit) {
		...
		if (!$product_data) {
			$query = $this->db->query("SELECT p.product_id FROM " . DB_PREFIX . "product p LEFT JOIN " . DB_PREFIX . "product_to_store p2s ON (p.product_id = p2s.product_id) WHERE p.status = '1' AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "' ORDER BY CAST(p.model AS UNSIGNED) DESC LIMIT " . (int)$limit);
	...
	public function getSizesHelpImage($product_id) {
		$strurl = "http://bizoutmax.ru/index.php?route=product/product&product_id=" . (string)$product_id;

		$page = file_get_contents($strurl);

		if (strpos($page, 'product-info') === false)
			return 'no_image';

		// $sizeImageUrl = $this->getTextBetween($page, '<h2>Выберите размер: <a class="colorbox" href="', '" style=');
		$sizeImageUrl = $this->getTextBetween($page, 'id="product-help-size-image">', '</span>');

		if ($sizeImageUrl == "") {
			$sizeImageUrl = $this->getTextBetween($page, '<h2>Выберите размер: <a class="colorbox" href="', '">Помочь выбрать >><');
		}

		if ($sizeImageUrl != "") {
			// $this->loadImageFromHost("http://bizoutmax.ru/" . $sizeImageUrl, DIR_IMAGE .  str_replace("/image/", '', $sizeImageUrl));
			$this->loadImageFromHost("http://bizoutmax.ru/" . $sizeImageUrl, DIR_IMAGE .  str_replace("images/", '', $sizeImageUrl));
			$sizeImageUrl = str_replace("images/", 'image/', $sizeImageUrl);
		} else {
			$sizeImageUrl = 'no_image';
		}

		$this->db->query("UPDATE " . DB_PREFIX . "product SET sizes_help_image = '" . $sizeImageUrl . "' WHERE product_id = '" . (int)$product_id . "'");
		return $sizeImageUrl;
	}

	public function loadImageFromHost($link, $img_path) {
		$pathArray = explode('/', $img_path, -1);
	...
	public function getTextBetween($text, $leftBorder, $rightBorder = null) {
